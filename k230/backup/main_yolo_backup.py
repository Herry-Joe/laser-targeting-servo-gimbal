...
#   v4.15: 无激光时不发误差(valid=False), STM32 保持不动.
#          反光拒识: 长宽比≤2.5 + 紧凑度≥0.3, 筛掉长条状噪声.
#          debug 输出: 每30帧打印激光检测状态/质量.
#   v4.14: 激光检测升级为多阈值+局部ROI+最近邻跟踪, 解决不同位置颜色阈值漂移问题
...
def detect_laser(laser_img, prev_x, prev_y, locked):
    """多阈值+局部ROI+最近邻跟踪的激光检测 (v4.14).
    
    解决 "激光打到不同位置阈值会变" 的问题:
      1. 同时搜索多组 LAB 阈值(标准红 + 用户实测偏色范围), 覆盖颜色漂移.
      2. 锁定后只在上一帧位置附近的小 ROI 内搜索, 减少背景/反光误检.
      3. 锁定后优先选离上一帧位置最近的 blob, 而不是最大面积, 抑制跳变.
    ...
    for b in blobs:
        ...
        if bw < 1 or bh < 1:
            continue
        # 长宽比过滤: 反光/噪声常为长条状, 激光点接近圆形
        ratio = float(max(bw, bh)) / float(min(bw, bh))
        if ratio > 2.5:
            continue
        # 紧凑度过滤: 真实激光点应填充 >30% 的 bounding box
    ...
    if locked and prev_x >= 0 and prev_y >= 0:
        ...
        best_data = max(cands, key=lambda t: t[1] * t[3])  # 面积×紧凑度评分
...
def run():
    ...
    try:
        ...
        l_lock=False; l_hit_cnt=0; l_miss_cnt=0; lx_s=0; ly_s=0  # v4.12: 激光时序确认+EMA状态
        ...
        while True:
            ...
            # --- 激光检测 (800x480) — v4.14: 多阈值+局部ROI+最近邻跟踪 ---
            # 同时搜索标准红+用户实测偏色阈值; 锁定后局部ROI减少背景误检; 最近邻抑制跳变
            raw_det=False; rlx=rly=-1
            if laser_img:
            ...
            if raw_det:
                ...
                if l_hit_cnt >= LASER_LOCK_FRAMES:
                    if not l_lock:
                        l_lock = True
                    last_lx = lx_s; last_ly = ly_s; last_laser_ms = time.ticks_ms()
            else:
            ...
            if fc % 30 == 0:
                print("[LASER] det=%d lock=%d hit=%d miss=%d pos=(%d,%d)" % (
                    raw_det, l_lock, l_hit_cnt, l_miss_cnt, lx if l_det else -1, ly if l_det else -1))
                if raw_det and not l_lock:
                    print("[LASER] 原始帧命中但未锁定(需连续 %d 帧)" % LASER_LOCK_FRAMES)
                elif l_lock and l_miss_cnt > 0:
                    print("[LASER] 锁定中, 已丢失 %d 帧(超 %d 释放)" % (l_miss_cnt, LASER_LOST_FRAMES))
            ...
            if l_det:
                ...
                if not raw_det:  # 时序保持中(当前帧未raw命中) → 空心环提示
                    osd.draw_circle(lx,ly,6,C_LASER,1)
            ...
            # --- 误差 → 串口 (v4.15: 无激光不发误差) ---
            # 核心原则: 激光没识别到就发 valid=0, STM32 保持不动
            now=time.ticks_ms()
            TARGET_HOLD_MS = 400
            if target_valid:
                last_tgt_ms = now
            ...
            if l_det_hold and target_valid:
                ex = tgt_x - vlx; ey = tgt_y - vly       # 激光追靶心 (正常追踪)
                valid_out = True
            elif l_det_hold and not target_valid:
                ex = 0; ey = 0; valid_out = True         # 激光在但无靶心 → 发送 (0,0) 保持位置
            else:
                ex = 0; ey = 0; valid_out = False        # 无激光 → 不发误差, STM32 保持不动
            ...
            # 调试
            if fc%60==0:
                print("[DBG] FPS=%.1f T=(%d,%d)[%s] err=(%d,%d) v=%d" % (
                    fps, tgt_x, tgt_y, target_from,
                    sx_err, sy_err, valid_out))
