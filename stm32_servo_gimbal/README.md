...
| 6 | POSCTRL | **位置闭环**（默认）|
| 7 | TRACE | 矩形循迹测试（仅作模式枚举；实际视觉循迹由 K230 在 AUTO 内驱动，见下）|

### AUTO 模式（K230 自动打靶 + 矩形循迹）
...
#### 矩形循迹（v5.7：[v5.7] 找四角 + 拐角减速 + 直线提速）

- 进入方式：按 **PA1** → STM32 保持 AUTO 模式并向 K230 发 `TRACE_BORDER` 命令帧；**K230 解析命令后切到循迹任务**（task_id=2 外框 / 3 内框），STM32 始终按 K230 误差帧闭环驱动云台（注意：**不再用 open-loop 的 GIMBAL_TRACE**，旧版会忽略 K230 帧、只是开环画方框，无法真正循迹）。
- **四角减速 / 直线提速**：K230 在 setpoint 接近四个角时置 **FLAG bit5 = NEAR_CORNER**，STM32 解析后置 `g_k230_corner`：
  - 直线段（非拐角）：速度地板 `TRACE_SPEED=0.60`，比旧版（固定 0.7×且被减速区压死）快，画直线流畅；
  - 拐角附近：速度地板降到 `TRACE_SPEED×CORNER_FACTOR=0.21`，强减速防过冲冲出角外。
  - 循迹时**解除减速区限幅**（dec_zone 拉大），改由速度地板控制，避免直线段被 throttle 拖慢。
- **不摇摆 / 不微抖**：定点锁靶新增**停止带** `VEL_STOP_BAND=0.015`——非循迹且速度指令低于阈值直接停机并锁靶，彻底消除靶心附近的"爬行微抖 / 摇摆不定"；循迹时持续跟随不锁靶。速度 EMA 改为带符号 `0.6/0.4`，方向跟随误差符号、err≈0 时沿用历史方向（避免强制单向漂移）。
- PA0 短按 = 回中/停止，长按 = 急停。
...
- **K230 端**（见 `../k230/README.md`）
  ...
  - 通信延迟降低，命中即锁存停机
- **v5.7 矩形循迹 / 抗过冲 / 抗亮度变化（本轮）**
  - **矩形循迹真正打通**：PA1 改为"STM32 保持 AUTO + 向 K230 发 `TRACE_BORDER` 命令帧"，K230 解析命令后切循迹任务（task_id=2/3），STM32 始终按 K230 误差帧闭环驱动。（旧版 PA1 走 open-loop `GIMBAL_TRACE`，忽略了 K230 帧、只是开环画方框，无法真正循迹）
  - **四角减速 + 直线提速**：K230 接近四角置 **FLAG bit5 = NEAR_CORNER** → STM32 `g_k230_corner`；直线段速度地板 `TRACE_SPEED=0.60`（比旧版快），拐角降到 `0.21` 强减速防过冲；循迹时解除减速区限幅改由速度地板控制。
  - **不摇摆**：定点锁靶新增停止带 `VEL_STOP_BAND=0.015`，非循迹且速度指令过低直接停机锁靶，消除靶心附近微抖/摇摆；速度 EMA 改带符号 `0.6/0.4`，方向跟随误差符号、err≈0 沿用历史方向。
  - **K230 抗亮度变化**：LAB 阈值表升级 5 级渐进式，新增 `RED_LASER_LAB_DIM (58,84,16,37,-10,9)` 优先档，`LASER_MIN_PX` 3→2；`RECT_HOLD_FRAMES` 5→6、`DETECT_INTERVAL` 2→3 提高识别/传输频率，视觉闭环更流畅。
  - 产物 `servo_gimbal.hex` 已重新生成（objcopy 走英文临时路径规避中文路径写盘问题）。

- **v5.8 循迹精度提升（本轮）**
  - **消极限环（抖动/稳态偏移）**：循迹速度地板由"恒定 `TRACE_SPEED=0.60`"改为**误差相关**——`|err| > TRACE_FLOOR_ERR_PX(25)` 时用 `0.60` 远线跟线速度地板，`|err| ≤ 25` 时降到 `TRACE_NEAR_FLOOR=0.06`，允许误差归零、激光贴线不抖。数值仿真验证：误差 RMS 由 ~72px 降至 ~32px（2.3×），平均误差 37→28px。
  - 依据：用 `k230/trace_sim.py` 离线复现闭环（控制环 + 周长行走 + 检测模型），扫描确认 `TRACE_SPEED=0.60` 为最优远线地板（过低则跟不上行走速度、误差反而增大），近线小地板是消极限环关键。
  - 配合 K230 端 `detect_laser` 检测门限（拒远处反射误检），可将 catastrophic 假误差尖峰（实测曾达 ~700px）砍到 <80px。

- **v5.9 自动打靶 Pan 轴卡死 + AIM 模式摇摆修复（本轮）**
  - **Pan 轴双向锁死（致命）**：`K230_ControlAxis` 软限位旧逻辑 `next>+LIM || next<-LIM` 在电机**已停在限位外**（位置环先把它带到 ±LIM 之外，实测 Pan 停到 450 步 > `PAN_MAX_STEPS=400`）时，CW 与 CCW 均被挡 → 该轴永远动不了，X 误差恒为 ~-226px。改为**仅禁止进一步向外、允许从限位外向中心回拉**；并把 `PAN_MAX_STEPS` 400→450（与位置环实测可达 ±45° 一致）。
  - **AIM 模式摇摆（噪声反向）**：`K230_HYST_PX` 4→12（K230 误差噪声约 ±15px，迟滞需大于噪声才不反复解锁反向）；停止带不再 `vel_ema=0`，保留上次方向记忆，避免误差小幅反向时反复启停。循迹模式不受影响（跳过锁靶逻辑）。
