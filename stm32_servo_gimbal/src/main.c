                              ...
                              float max_speed_scale)
{
    ...
    if (abs(err) > deadzone_px) {
        ...
        /* 循迹任务(外框 task_id=2 / 内框 task_id=3)降速 0.7x: 激光更贴线, 减少冲出线外 */
        if (g_k230_task_id == 2 || g_k230_task_id == 3) {
            vel_mag *= 0.7f;
        }

        /* 速度指令 EMA (加速度限幅): 平滑速度变化, 抑制矩形拐角处方向突变导致的过冲.
         * 0.55/0.45 兼顾跟手与抑抖, 约 5 帧(≈0.16s @30fps)达到目标速度. */
        if (*vel_ema == 0.0f) *vel_ema = vel_mag;
        else *vel_ema = *vel_ema * 0.55f + vel_mag * 0.45f;
        vel_mag = *vel_ema;
...
static void PA1_ShortPress(void)
{
    if (g_gimbal_mode == GIMBAL_TRACE) {
        g_trace_active = 0;
        Stepper_Stop(&g_motor_pan); Stepper_Stop(&g_motor_tilt);
        g_gimbal_mode = GIMBAL_POSCTRL;
        printf("\r\n[PA1] 停止矩形循迹 → 位置闭环\r\n");
    } else {
        Trace_Init(TRACE_SIZE_DEFAULT);
        printf("\r\n[PA1] → 矩形循迹(TRACE ±%d步)\r\n", TRACE_SIZE_DEFAULT);
    }
}
