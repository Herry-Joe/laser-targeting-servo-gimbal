...
> ## ⚠️ 舵机版（Servo）已就绪
> 本项目已改造为 **舵机二维云台 + 空心滑环** 方案：双轴 270° 标准舵机由 **TIM3 50Hz PWM**（PA6=Pan / PA7=Tilt）驱动，Pan 轴经滑环连续旋转。固件已编译通过，产物 `servo_gimbal.hex`。
> 硬件改动、引脚重分配、角度映射、固件改动与烧录方法，见 **[`servo_gimbal_design.md`](./servo_gimbal_design.md)**。
> 下方原文档保留为步进版参考。
